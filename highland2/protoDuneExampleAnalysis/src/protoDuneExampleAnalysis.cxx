#include "protoDuneExampleAnalysis.hxx"
...
/*
  ...
  Then protoDuneExampleAnalysis inherits from baseAnalysis, which inherits from AnalysisAlgorithm.
...
//********************************************************************
protoDuneExampleAnalysis::protoDuneExampleAnalysis(AnalysisAlgorithm* ana) : baseAnalysis(ana) {
//********************************************************************

  // Add the package version
  ND::versioning().AddPackage("protoDuneExampleAnalysis", anaUtils::GetSoftwareVersionFromPath((std::string)getenv("DUNEEXAMPLEANALYSISROOT")));
}

//********************************************************************
bool protoDuneExampleAnalysis::Initialize(){
//********************************************************************
  ...
  */
  ...
  SetMinAccumCutLevelToSave(ND::params().GetParameterI("protoDuneExampleAnalysis.MinAccumLevelToSave"));
...
//********************************************************************
  ...
  */
  ...
  sel().AddSelection("pionSelection",            "protoDuneExample selection",     new pionSelection(false));     // true/false for forcing break  
  /*
...
//********************************************************************
void protoDuneExampleAnalysis::DefineCorrections(){
//********************************************************************
  ...
  */
  ...
  if (ND::params().GetParameterI("protoDuneExampleAnalysis.Corrections.EnabledEdx"))             corr().AddCorrection("dEdx",              new dEdxCorrection());
  if (ND::params().GetParameterI("protoDuneExampleAnalysis.Corrections.EnabledEdxData"))         corr().AddCorrection("dEdx data",         new dEdxDataCorrection());   
  if (ND::params().GetParameterI("protoDuneExampleAnalysis.Corrections.EnabledBeamMom"))         corr().AddCorrection("BeamMom",           new BeamMomCorrection());
  if (ND::params().GetParameterI("protoDuneExampleAnalysis.Corrections.EnabledBeamMomSmearing")) corr().AddCorrection("BeamMomSmearing",   new BeamMomSmearingCorrection());
  if (ND::params().GetParameterI("protoDuneExampleAnalysis.Corrections.EnabledCryoWallBeamMom")) corr().AddCorrection("CryoWallBeamMom",   new CryoWallBeamMomCorrection());
  if (ND::params().GetParameterI("protoDuneExampleAnalysis.Corrections.EnabledBrokenTrackData")) corr().AddCorrection("broken track data", new BrokenTrackDataCorrection());   
...
//********************************************************************
void protoDuneExampleAnalysis::DefineConfigurations(){
//********************************************************************
  ...
  */
  ...
  if (_enableAllSystConfig){
    if (ND::params().GetParameterI("protoDuneExampleAnalysis.Variations.EnabledEdx"))        conf().EnableEventVariation(SystId::kdEdx,         all_syst);
  }

}

//********************************************************************
void protoDuneExampleAnalysis::DefineMicroTrees(bool addBase){
//********************************************************************
...
//********************************************************************
void protoDuneExampleAnalysis::DefineTruthTree(){
//********************************************************************
...
//********************************************************************
void protoDuneExampleAnalysis::FillMicroTrees(bool addBase){
//********************************************************************
  ...
  */
  ...
  if (beamPart){
    if (beamPart->TrueObject){
      ...
      //      AnaTrueParticle* beamTruePart= protoDuneSelUtils::FindBeamTrueParticle(GetSpill());
  ...
  if (box().MainTrack){
    ...
    if (box().MainTrack->Original){
      if (box().MainTrack->Original->Original){
        output().FillVar(seltrk_pida_corr,  protoDuneSelUtils::ComputePIDA(*static_cast<const AnaParticle*>(box().MainTrack->Original->Original)));      
      }
      if (box().MainTrack->Original->Original->Original){
        output().FillVar(seltrk_pida_raw,  protoDuneSelUtils::ComputePIDA(*static_cast<const AnaParticle*>(box().MainTrack->Original->Original->Original)));
      }
    }
    ...
    for (Int_t i=0;i<std::min((Int_t)100,ndau); ++i){
      ...
      protoDuneSelUtils::ComputeBinnedDeDx(dau,20,4,dau_binned_dedx);
    ...
    if(beamPart){
      ...
      output().FillVar(seltrk_csdarange_muon,         protoDuneSelUtils::ComputeCSDARange(1000*(beamPart->Momentum), 13));
      output().FillVar(seltrk_csdarange_prot,         protoDuneSelUtils::ComputeCSDARange(1000*(beamPart->Momentum), 2212));
      if (beamPart->TrueObject){
        output().FillVar(seltrk_csdarange_muon_raw,     protoDuneSelUtils::ComputeCSDARange(1000*(static_cast<AnaTrueParticle*>(beamPart->TrueObject)->Momentum), 13));
        output().FillVar(seltrk_csdarange_prot_raw,     protoDuneSelUtils::ComputeCSDARange(1000*(static_cast<AnaTrueParticle*>(beamPart->TrueObject)->Momentum), 2212));
      }
      else{
        output().FillVar(seltrk_csdarange_muon_raw,     protoDuneSelUtils::ComputeCSDARange(1000*(static_cast<const AnaParticle*>(beamPart->Original->Original)->Momentum), 13));
        output().FillVar(seltrk_csdarange_prot_raw,     protoDuneSelUtils::ComputeCSDARange(1000*(static_cast<const AnaParticle*>(beamPart->Original->Original)->Momentum), 2212));
      }
      output().FillVar(seltrk_csdarange_tpc_muon,     protoDuneSelUtils::ComputeCSDARange(1000*((Float_t)beam->BeamMomentumInTPC), 13));
      output().FillVar(seltrk_csdarange_tpc_prot,     protoDuneSelUtils::ComputeCSDARange(1000*((Float_t)beam->BeamMomentumInTPC), 2212));
    }


    //    output().FillVar(seltrk_mom_muon2,         protoDuneSelUtils::ComputeRangeMomentum(box().MainTrack->Length/10., 13));
    //    output().FillVar(seltrk_mom_prot2,         protoDuneSelUtils::ComputeRangeMomentum(box().MainTrack->Length/10., 2212));
    ...
    Float_t posz[3]={0};
    output().FillVectorVarFromArray(seltrk_pos_z0, protoDuneSelUtils::ExtrapolateToZ(box().MainTrack,0,posz), 3); //default extrapolates to 0
    ...
    //    output().FillVar(seltrk_mom_muon_z0,           protoDuneSelUtils::ComputeRangeMomentum(box().MainTrack->Length + diflength, 13));
    //    output().FillVar(seltrk_mom_prot_z0,           protoDuneSelUtils::ComputeRangeMomentum(box().MainTrack->Length + diflength, 2212));

    output().FillVar(seltrk_kinetic, protoDuneSelUtils::ComputeKineticEnergy(*box().MainTrack));              
    ...
    protoDuneSelUtils::ComputeBinnedDeDx(static_cast<const AnaParticle*>(box().MainTrack),20,4,binned_dedx);
    for (int i=0;i<3;i++){
...
//********************************************************************
void protoDuneExampleAnalysis::FillToyVarsInMicroTrees(bool addBase){
//********************************************************************
  ...
  */
  ...
  if (box().MainTrack){
    output().FillToyVar(seltrk_pida,  protoDuneSelUtils::ComputePIDA(*box().MainTrack));
  }
  
}
//********************************************************************
bool protoDuneExampleAnalysis::CheckFillTruthTree(const AnaTrueVertex& vtx){
//********************************************************************
...
//********************************************************************
void protoDuneExampleAnalysis::FillTruthTree(const AnaTrueVertex& vtx){
//********************************************************************
  ...
  AnaTrueParticle* beampart= protoDuneSelUtils::FindBeamTrueParticle(GetSpill());
...
//********************************************************************
void protoDuneExampleAnalysis::FillCategories(){
//********************************************************************
