...
//********************************************************************
AnaSubdet1ParticleB::AnaSubdet1ParticleB(const AnaSubdet1ParticleB& seg):AnaParticleB(seg){
//********************************************************************
}

//********************************************************************
void AnaSubdet1ParticleB::Print() const{
//********************************************************************

    std::cout << "-------- AnaSubdet1ParticleB --------- " << std::endl;
...
//********************************************************************
AnaSubdet2ParticleB::AnaSubdet2ParticleB():AnaParticleMomB() {
//********************************************************************
...
//********************************************************************
AnaSubdet2ParticleB::AnaSubdet2ParticleB(const AnaSubdet2ParticleB& seg):AnaParticleMomB(seg){
//********************************************************************
...
//********************************************************************
void AnaSubdet2ParticleB::Print() const{
//********************************************************************

    std::cout << "-------- AnaSubdet2ParticleB --------- " << std::endl;

    AnaParticleMomB::Print();
}

//********************************************************************
AnaSubdet2ParticleB::~AnaSubdet2ParticleB(){
//********************************************************************
}
...
//********************************************************************
    ...
    Subdet1SegmentsVect.clear();
    Subdet2SegmentsVect.clear();
}
...
//********************************************************************
    ...
    if (Subdet1SegmentsVect.size()>0  && nSubdet1Segments ==0) for (UInt_t i=0;i<Subdet1SegmentsVect.size(); i++) delete Subdet1SegmentsVect[i];
    if (Subdet2SegmentsVect.size()>0  && nSubdet2Segments ==0) for (UInt_t i=0;i<Subdet2SegmentsVect.size(); i++) delete Subdet2SegmentsVect[i];
...
//********************************************************************
    ...
    nSubdet1Segments  = track.nSubdet1Segments;
    nSubdet2Segments  = track.nSubdet2Segments;


    for (Int_t i=0;i<track.nSubdet1Segments;i++)
        Subdet1Segments[i] = track.Subdet1Segments[i]->Clone();

    for (Int_t i=0;i<track.nSubdet2Segments;i++)
        Subdet2Segments[i] = track.Subdet2Segments[i]->Clone();
...
//********************************************************************
    ...
    anaUtils::CopyArray(truePart.DirectionEnd, DirectionEnd, 3);

    Momentum   = truePart.Momentum;
    MomentumEnd= truePart.MomentumEnd;
...
//********************************************************************
    ...
    std::cout << "ProcessStart: " << ConvertProcess(ProcessStart) << std::endl;
    std::cout << "ProcessEnd:   " << ConvertProcess(ProcessEnd) << std::endl;
...
//*****************************************************************************
  ...
  for (std::vector<AnaBunchC*>::iterator it=bunches.begin();it!=bunches.end();it++){
    ...
    for (UInt_t i=0;i<bunch->Particles.size();i++){
      AnaTrackB* track = dynamic_cast<AnaTrackB*>(bunch->Particles[i]);
      if (!track)  continue;
      track->nSubdet1Segments=0;
      for (UInt_t j=0;j<track->Subdet1SegmentsVect.size();j++)
        track->Subdet1Segments[track->nSubdet1Segments++]=track->Subdet1SegmentsVect[j];

      track->nSubdet2Segments=0;
      for (UInt_t j=0;j<track->Subdet2SegmentsVect.size();j++)
        track->Subdet2Segments[track->nSubdet2Segments++]=track->Subdet2SegmentsVect[j];
    }
...
//*********************************************************
std::string AnaTrueParticleB::ConvertProcess(AnaTrueParticleB::ProcessEnum process) const{
//*********************************************************
...
//*********************************************************
AnaTrueParticleB::ProcessEnum AnaTrueParticleB::ConvertProcess(const std::string& sprocess) const{
//*********************************************************
