...
public:
  ...
  // Set experiment-specific event display implementation
  void SetEventDisplay(EventDisplayBase* ed) { _eventDisplay = ed; }
  ...
  void EvtDisplay(const std::string& mode, Int_t run, Int_t subrun, Int_t evt, const std::string& outputFile = "");

  // Simple interface without mode (defaults to multi-view display)
...
protected:
  ...
  EventDisplayBase* _eventDisplay;
  bool _autoCreatedEventDisplay;

private:
  // Auto-create event display based on class name stored in file
  EventDisplayBase* AutoCreateEventDisplay();
