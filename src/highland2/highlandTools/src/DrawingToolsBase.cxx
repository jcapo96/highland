...
//********************************************************************
  ...
  // default position and option for Stats box
  ResetStat();
...
//*********************************************************
  ...
  // Draw 1D histogram for a given set of cuts and uniform binning
  double xbins[NMAXBINS];
  return Draw(tree, var, nx, GetVariableBins(nx,xmin,xmax,xbins), categ, cut, root_opt, opt, leg, norm, scale_errors);
}
...
//*********************************************************
  ...
  int ny=0;
  double* ybins=NULL;
  return Draw(tree,var,nx,xbins,ny,ybins,categ,cut, root_opt, opt, leg, norm,scale_errors);
}
...
TH1* DrawingToolsBase::Draw(TTree* tree, const std::string& var, int nx, double* xbins, int ny, double* ybins, const std::string& categ,
                            const std::string& cut,  const std::string& root_opt, const std::string& opt, const std::string& leg, double norm2,  bool scale_errors){
//*********************************************************
  ...
  // Create the legend // TODO: Anselmo 
  //  if (!drawUtils::CheckOption(uopt,"NOLEG"))
  if (!drawUtils::CheckOption(uopt,"NOLEG") && uroot_opt.find("SAME") == std::string::npos)
    CreateLegend(uopt);
  ...
  //Draw the HistoStack
  DrawHistoStack(hs,categ,root_opt, uopt,leg);
...
//**************************************************
void DrawingToolsBase::DrawHistoStack(HistoStack* hs, const std::string& categ, const std::string& root_opt, const std::string& opt,const std::string& leg,const std::string& leg_opt,int mode){
//**************************************************

  DrawHistoStack(hs,categ,root_opt,opt,_line_color, _line_width, _fill_color, _fill_style, leg, leg_opt, mode);
}

//**************************************************
void DrawingToolsBase::DrawHistoStack(HistoStack* hs, const std::string& categ, const std::string& root_opt, const std::string& opt, int lc, int lw, int fc, int fs, const std::string& leg,const std::string& leg_opt,int mode){
//**************************************************
...
//*********************************************************
void DrawingToolsBase::DrawCutLinesVertical(double xval_min, double xval_max, bool addarrow, double arrowpos) {
//*********************************************************

  DrawCutLineVertical(xval_min, addarrow, "r", arrowpos);
  DrawCutLineVertical(xval_max, addarrow, "l", arrowpos); 
}
...
//*********************************************************
  ...
  char highlandpath[200]="unknown";
  char hostname[50]="unknown";
  char inputFile[200]="unknown";
  char OriginalFile[200]="unknown";
  if (drawUtils::TreeHasVar(chain,"HOSTNAME")){
    chain->SetBranchAddress("HIGHLANDPATH", highlandpath);
  ...
  std::cout << std::endl;
  std::cout << "HIGHLANDPATH:             " << highlandpath << std::endl;
...
//*********************************************************
void DrawingToolsBase::ResetStat(){
//*********************************************************
