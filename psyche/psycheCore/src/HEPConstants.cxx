...
namespace units{
  ...
  // baryons
  const double mass_sigma_plus   = pdgBase->GetParticle(3222)->Mass()*units::GeV;
  const double mass_sigma_minus  = pdgBase->GetParticle(3112)->Mass()*units::GeV;
  const double mass_sigma_zero   = pdgBase->GetParticle(3212)->Mass()*units::GeV;
}
